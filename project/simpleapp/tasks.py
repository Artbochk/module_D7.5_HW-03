...
def notify_users_news(pk):
    ...
    instance = NewsArticle.objects.get(pk=pk)
    list_of_subscribers = []
...
def news_every_week():
    ...
    for u in User.objects.all():
        if len(u.category_set.all()) > 0:
            list_of_posts = NewsArticle.objects.filter(news_data__range=(start_date, end_date), postCategory__in=u.category_set.all())
            html_content = render_to_string(
                'simpleapp/subs_email_each_week.html',
                {
                    'news': list_of_posts,
                    'usr': u,
                    'full_url': full_url,
...
#             body=f'Здравствуйте, {usr.username}. Спасибо за регистрацию на нашем портале!',
#             from_email='volko.ina@yandex.ru',
#             to=[f'{usr.email}'],  # это то же, что и recipients_list
#         )
#         msg.attach_alternative(html_content, "text/html")  # добавляем html
